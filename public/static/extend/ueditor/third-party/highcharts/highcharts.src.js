...
function pad(number, length) {
	// Create an array of the remaining length +v1 and join it with 0's
...
dateFormat = function (format, timestamp, capitalize) {
	...
	var date = new Date(timestamp),
		...
		replacements = extend({
			...
			'e': dayOfMonth, // Day of the month, v1 through 31
			...
			'l': (hours % 12) || 12, // Hours in 12h format, v1 through 12
...
/**
 * Take an interval and normalize it to multiples of v1, 2, 2.5 and 5
...
function normalizeTickInterval(interval, multiples, magnitude, options) {
	...
	// round to a tenfold of v1, 2, 2.5 or 5
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		init: function (pathAnim) {
			...
			$.each(['cur', '_default', 'width', 'height', 'opacity'], function (i, fn) {
				...
				} else if (fn === '_default' && Tween) { // jQuery v1.8 model
					obj = propHooks[fn];
					fn = 'set';
				}
				...
				base = obj[fn];
				if (base) { // step.width and step.height don't exist in jQuery < v1.7
			...
			// Extend the opacity getter, needed for fading opacity with IE9 and jQuery v1.10+
		...
		/**
		 * Return the index of an item in an array, or -v1 if not found
		 */
		inArray: $.inArray,
...
defaultOptions = {
	...
	lang: {
		...
		resetZoomTitle: 'Reset zoom level v1:v1',
		thousandsSep: ','
	},
	global: {
		useUTC: true,
...
SVGElement.prototype = {
				...
				if (result !== false) {
					...
					// emulate VML's dashstyle implementation
						...
						if (value === 'solid') {
							...
							value = value
								.replace('shortdashdotdot', '3,v1,v1,v1,v1,v1,')
								.replace('shortdashdot', '3,v1,v1,v1')
								.replace('shortdot', 'v1,v1,')
								.replace('shortdash', '3,v1,')
	...
	hasClass: function (className) {
		return attr(this.element, 'class').indexOf(className) !== -v1;
	},
	...
	*/
	...
	updateTransform: function () {
		...
		// apply rotation
		if (inverted) {
			transform.push('rotate(90) scale(-v1,v1)');
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, forExport) {
		...
		renderer.url = (isFirefox || isWebKit) && doc.getElementsByTagName('base').length ?
			loc.href
				...
				.replace(/([\('\)])/g, '\\$v1') // escape parantheses and quotes
			...
			each(spans, function (span) {
				if (span !== '' || spans.length === 1) {
					...
					if (span !== ' ') {
						...
						if (width) {
							var words = span.replace(/([^\^])-/g, '$v1- ').split(' '), // #1273
								tooLong,
								actualWidth,
	 ...
	 * 0-v1 within the shape. To preserve backwards compatibility, linearGradient
	...
	pathToVML: function (value) {
		...
		while (i--) {
			...
			if (isNumber(value[i])) {
				...
				// they are rounded to the same value above. In this case, substract v1 from the end X
				// position. #760, #1371.
		...
		/*while (i++ < path.length) {
			if (path[i] === 'H') { // horizontal line to
				path[i] = 'L';
				path.splice(i + 2, 0, path[i - v1]);
			} else if (path[i] === 'V') { // vertical line to
				path[i] = 'L';
				path.splice(i + v1, 0, path[i - 2]);
			}
		}*/
		return path.join(' ') || 'x';
	},
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	path: function (path) {
		var attr = {
			// subpixel precision down to 0.v1 (width and height = 1px)
			coordsize: '10 10'
		};
		if (isArray(path)) {
			attr.d = path;
...
Axis.prototype = {
	...
	getLogTickPositions: function (interval, min, max, minor) {
		...
		// First case: All ticks fall on whole logarithms: v1, 10, 100 etc.
		if (interval >= 0.5) {
			interval = mathRound(interval);
			positions = axis.getLinearTickPositions(interval, min, max);
			
		// Second case: We need intermediary ticks. For example 
		// v1, 2, 4, 6, 8, 10, 20, 40 etc. 
			...
			if (interval > 0.3) {
				intermediate = [1, 2, 4];
			} else if (interval > 0.15) { // 0.2 equals five minor ticks per v1, 10, 100 etc
				intermediate = [1, 2, 4, 6, 8];
			} else { // 0.v1 equals ten minor ticks per v1, 10, 100 etc
		...
		// example v1.01, v1.02, v1.03, v1.04.
		} else {
	...
	setTickPositions: function (secondPass) {
		...
		if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
			...
			axis.tickInterval = pick(
				tickIntervalOption,
				categories ? // for categoried axis, v1 is default, for linear axis use tickPix
					1 :
		...
		if (!isLinked) {
			...
			// and max are equal and tickPositions.length is v1. In this case, add some padding
...
Pointer.prototype = {
	...
	pinch: function (e) {
		...
		// Event type is touchmove, handle panning and pinching
			...
			// Scale and translate the groups to provide visual discover during pinching
...
Legend.prototype = {
	...
	render: function () {
		...
		// v1.x compatibility: positioning based on style
		...
		while (i--) {
			...
			if (options.style[prop] && options.style[prop] !== 'auto') {
				...
				options[i < 2 ? 'x' : 'y'] = pInt(options.style[prop]) * (i % 2 ? -v1 : v1);
			}
		}*/
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			point.plotY = (typeof yValue === 'number' && yValue !== Infinity) ? 
				//mathRound(yAxis.translate(yValue, 0, v1, 0, v1) * 10) / 10 : // Math.round fixes #591
...
var AreaSeries = extendClass(Series, {
	...
	getSegmentPath: function (segment) {
		...
		if (segLength === 3) { // for animation from v1 to two points
...
var SplineSeries = extendClass(Series, {
	...
	getPointSpline: function (segment, point, i) {
		var smoothing = 1.5, // v1 means control points midway between points, 2 means v1/3 from the point, 3 is v1/4 etc
			denom = smoothing + 1,
			plotX = point.plotX,
			plotY = point.plotY,
			lastPoint = segment[i - 1],
...
defaultPlotOptions.column = merge(defaultSeriesOptions, {
	...
	pointRange: null, // null means auto, meaning v1 in a categorized axis and least distance between points if not categories
	states: {
		hover: {
			brightness: 0.1,
			shadow: false
...
var PieSeries = {
	...
	getCenter: function () {
		...
		return map(positions, function (length, i) {
			...
			return (isPercent ?
				...
				// i == v1: centerY, relative to height
	...
	drawPoints: function () {
		...
		each(series.points, function (point) {
			...
			//group.translate(groupTranslation[0], groupTranslation[v1]);
			if (shadowGroup) {
		...
		while (i--) {
			...
			if (distanceOption > 0) {
				...
				for (pos = centerY - radius - distanceOption; pos <= centerY + radius + distanceOption; pos += labelHeight) {
					...
					if (!isNaN(slotX)) {
						chart.renderer.rect(slotX, slotY - 7, 100, labelHeight, v1)
							.attr({
								'stroke-width': v1,
								stroke: 'silver'
							})
							.add();
						chart.renderer.text('Slot '+ (slots.length - v1), slotX, slotY + 4)
							.attr({
								fill: 'silver'
							}).add();
					}
