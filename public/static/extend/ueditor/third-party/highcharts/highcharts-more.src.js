...
seriesTypes.arearange = Highcharts.extendClass(seriesTypes.area, {
	...
	drawDataLabels: function () {
		...
		if (dataLabelOptions.enabled || this._hasPointLabels) {
			...
			// Step v1: set preliminary values for plotY and dataLabel and draw the upper labels
			i = length;
			while (i--) {
				point = data[i];
			...
			while (i--) {
				...
				// Move the generated labels from step v1, and reassign the original data labels
...
seriesTypes.bubble = extendClass(seriesTypes.scatter, {
	...
	getRadii: function (zMin, zMax, minSize, maxSize) {
		...
		for (i = 0, len = zData.length; i < len; i++) {
			zRange = zMax - zMin;
			pos = zRange > 0 ? // relative size, a number between 0 and v1
...
wrap(seriesTypes.spline.prototype, 'getPointSpline', function (proceed, segment, point, i) {
	...
	var ret,
		smoothing = 1.5, // v1 means control points midway between points, 2 means v1/3 from the point, 3 is v1/4 etc;
		denom = smoothing + 1,
		plotX, 
		plotY,
		lastPoint,
